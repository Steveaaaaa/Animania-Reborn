...
public final class AnimaniaRodentRenderer extends MobRenderer<AnimaniaRodent, LegacyAnimalModel<AnimaniaRodent>> {
    ...
    private final java.util.Map<String, LegacyAnimalModel<AnimaniaRodent>> ferretVariants = new java.util.HashMap<>();
    ...
    public AnimaniaRodentRenderer(EntityRendererProvider.Context context) {
        ...
        addLayer(new LegacyBlinkLayer<>(this, rodent -> "rodents/" +
                (rodent.kind() == AnimaniaRodent.Kind.HAMSTER ? "hamster_blink"
                        : rodent.kind().isFerret() ? (rodent.kind() == AnimaniaRodent.Kind.FERRET_CINNAMON
                                || rodent.kind() == AnimaniaRodent.Kind.FERRET_SABLE ? rodent.textureName() + "_blink" : "ferret_blink") : "hedgehog_blink"),
    ...
    public void render(AnimaniaRodent rodent, float yaw, float partialTick, PoseStack poseStack,
        ...
        if (rodent.kind() == AnimaniaRodent.Kind.FERRET_CINNAMON || rodent.kind() == AnimaniaRodent.Kind.FERRET_SABLE) {
            model = ferretVariants.computeIfAbsent(rodent.textureName(), key -> LegacyAnimalModel.loadVariant(
                    "extra/client/model/rodents/modelferret", "rodents/" + key));
        }
    ...
    private static int eyelidColor(AnimaniaRodent rodent) {
        ...
        if (rodent.kind() == AnimaniaRodent.Kind.FERRET_CINNAMON) return 0xB37749;
        if (rodent.kind() == AnimaniaRodent.Kind.FERRET_SABLE) return 0x554131;
