...
final class LegacyMotionContext {
    ...
    LegacyMotionContext(Entity entity) {
        this.entity = entity;
        eatTimer = ModAttachments.getData(entity, ModAttachments.SLEEPING) ? 0 : ModAttachments.getData(entity, ModAttachments.EATING_TICKS);
        pitch = ModAttachments.getData(entity, ModAttachments.SLEEPING) ? 0 : entity.getXRot();
        ticks = ModAttachments.getData(entity, ModAttachments.SLEEPING) ? 1 : entity.tickCount;
    }

    boolean getSleeping() { return ModAttachments.getData(entity, ModAttachments.SLEEPING); }
    ...
    boolean getFighting() { return ModAttachments.getData(entity, ModAttachments.FIGHTING); }
    String getRivalUniqueId() {
        String rival = ModAttachments.getData(entity, ModAttachments.RIVAL);
    ...
    float getHeadAnchorPointY(float partial) {
        ...
        int timer = ModAttachments.getData(entity, ModAttachments.EATING_TICKS);
        if (timer <= 0) return 0;
