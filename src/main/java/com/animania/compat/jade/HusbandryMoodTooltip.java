...
final class HusbandryMoodTooltip {
    ...
    static void write(CompoundTag data, Animal animal) {
        ...
        data.putInt("Mood", ModAttachments.getData(animal, ModAttachments.MOOD_SCORE));
        data.putInt("MoodMissing", ModAttachments.getData(animal, ModAttachments.MOOD_MISSING));
        data.putInt("MoodGrace", ModAttachments.getData(animal, ModAttachments.MOOD_GRACE));
        data.putInt("MoodEffect", HusbandryMood.effect(animal));
        data.putBoolean("MoodBreeding", HusbandryMood.breedingAllowed(animal));
        data.putInt("MilkRest", HusbandryMood.effect(animal) >= 0 ? 0 : ModAttachments.getData(animal, ModAttachments.MILK_REST));
    }
