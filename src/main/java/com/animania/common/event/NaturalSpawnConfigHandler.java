...
public final class NaturalSpawnConfigHandler {
    ...
    @SubscribeEvent
    public static void replaceNewNestBees(net.minecraftforge.event.level.ChunkEvent.Load event) {
        ...
        level.getServer().tell(new net.minecraft.server.TickTask(level.getServer().getTickCount() + 1, () -> {
            ...
            for (var pos : chunk.getBlockEntitiesPos()) {
                ...
                var tag = hive.saveWithFullMetadata();
                var bees = tag.getList("Bees", 10);
                boolean changed = false;
                for (int i = 0; i < bees.size(); i++) {
                    var data = bees.getCompound(i).getCompound("EntityData");
                ...
                if (changed) { hive.load(tag); hive.setChanged(); }
            }
        }));
    }

    private static boolean replaceModernAnimal(MobSpawnEvent.FinalizeSpawn event) {
