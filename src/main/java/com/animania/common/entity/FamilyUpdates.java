...
public final class FamilyUpdates extends SavedData {
    ...
    private static FamilyUpdates get(ServerLevel level) {
        return level.getServer().overworld().getDataStorage().computeIfAbsent(
                FamilyUpdates::load, FamilyUpdates::new, "animania_family_updates");
    }
    ...
    public static void weaned(Animal child) {
        ...
        String mother = ModAttachments.getData(child, ModAttachments.PARENT), id = ModAttachments.getData(child, ModAttachments.NURSING_ID);
        ...
        ModAttachments.setData(child, ModAttachments.NURSING_ID, ""); data.setDirty();
    }
    public static void died(Animal animal) {
        if (!(animal.level() instanceof ServerLevel level)) return;
        weaned(animal);
        String mate = ModAttachments.getData(animal, ModAttachments.LAST_MATE);
        if (!mate.isEmpty()) {
    ...
    public static void receive(Animal animal) {
        ...
        if (removed != null) {
            Set<String> children = new LinkedHashSet<>(Arrays.asList(ModAttachments.getData(animal, ModAttachments.NURSING_YOUNG).split(",")));
            children.remove(""); children.removeAll(removed);
            ModAttachments.setData(animal, ModAttachments.NURSING_YOUNG, String.join(",", children));
            if (children.isEmpty()) FamilyLifecycle.endLactation(animal);
            data.setDirty();
        }
        String deadMate = data.bereaved.remove(id);
        if (deadMate != null) {
            if (ModAttachments.getData(animal, ModAttachments.LAST_MATE).equals(deadMate)) ModAttachments.setData(animal, ModAttachments.LAST_MATE, "");
            data.setDirty();
        }
    }
}
