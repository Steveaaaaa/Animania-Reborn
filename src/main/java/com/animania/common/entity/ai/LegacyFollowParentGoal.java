...
public final class LegacyFollowParentGoal extends Goal {
    ...
    public boolean canUse() {
        ...
        if (inactiveTime || child.getData(ModAttachments.SLEEPING)) return false;
    ...
    public boolean canContinueToUse() {
        if (child.getData(ModAttachments.SLEEPING) || child.hurtTime > 0 || child.isLeashed()
    ...
    private static boolean sameBreed(Animal child, Animal parent) {
        ...
        if (child.getData(ModAttachments.PARENT).equals(parent.getUUID().toString())) return child.getClass() == parent.getClass();
