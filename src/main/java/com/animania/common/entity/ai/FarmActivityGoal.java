...
public final class FarmActivityGoal extends LegacySearchBlockGoal {
    ...
    private boolean available() {
        return subject.isAlive() && subject.onGround() && !subject.isInWaterOrBubble()
                ...
                && !ModAttachments.getData(subject, ModAttachments.SLEEPING) && !LegacySleepGoal.shouldSleepNow(subject)
                && (acting || ModAttachments.getData(subject, ModAttachments.EATING_TICKS) == 0)
    ...
    @Override protected void onArriveAtDestination() {
        ...
        ModAttachments.setData(subject, ModAttachments.FARM_ACTIVITY, activity);
        ModAttachments.setData(subject, ModAttachments.FARM_ACTIVITY_START, (int) level.getGameTime());
    }
    ...
    @Override public void stop() {
        ModAttachments.setData(subject, ModAttachments.FARM_ACTIVITY, 0);
        acting = false;
        remaining = 0;
