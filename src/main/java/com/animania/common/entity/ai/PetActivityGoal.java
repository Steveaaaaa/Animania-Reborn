...
public final class PetActivityGoal extends Goal {
    ...
    private boolean available() {
        ...
        return pet.isAlive() && pet.hurtTime == 0 && pet.getTarget() == null && !pet.isOnFire()
                ...
                && !pet.getData(ModAttachments.SLEEPING) && !LegacySleepGoal.shouldSleepNow(pet)
                && pet.getData(ModAttachments.EATING_TICKS) == 0
    ...
    private void beginActivity() {
        ...
        pet.setData(ModAttachments.FARM_ACTIVITY, activity);
        pet.setData(ModAttachments.FARM_ACTIVITY_START, (int) pet.level().getGameTime());
    }
    ...
    @Override public void stop() {
        pet.setData(ModAttachments.FARM_ACTIVITY, 0);
        pet.getNavigation().stop();
