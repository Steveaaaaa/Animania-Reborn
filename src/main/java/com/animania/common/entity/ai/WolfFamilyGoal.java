...
public final class WolfFamilyGoal extends Goal {
    ...
    private static boolean available(AnimaniaDog animal) {
        return animal.breed().isWolf() && animal.isAlive() && !animal.isBaby() && !animal.isTame()
                ...
                && !animal.isInWaterOrBubble() && !ModAttachments.getData(animal, ModAttachments.SLEEPING)
                && ModAttachments.getData(animal, ModAttachments.EATING_TICKS) == 0
    ...
    private boolean follows(AnimaniaDog other) {
        String parent = ModAttachments.getData(wolf, ModAttachments.PARENT);
        String otherParent = ModAttachments.getData(other, ModAttachments.PARENT);
        ...
        boolean mates = ModAttachments.getData(wolf, ModAttachments.LAST_MATE).equals(other.getUUID().toString())
                && ModAttachments.getData(other, ModAttachments.LAST_MATE).equals(wolf.getUUID().toString());
    ...
    @Override public void start() {
        ...
        ModAttachments.setData(wolf, ModAttachments.FARM_ACTIVITY, FarmActivityGoal.FAMILY_REGROUP);
        ModAttachments.setData(wolf, ModAttachments.FARM_ACTIVITY_START, (int) wolf.level().getGameTime());
    }
    ...
    @Override public void stop() {
        ...
        ModAttachments.setData(wolf, ModAttachments.FARM_ACTIVITY, 0);
