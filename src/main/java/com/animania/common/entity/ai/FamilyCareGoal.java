...
public final class FamilyCareGoal extends Goal {
    ...
    private boolean available(Animal animal) {
        return animal.isAlive() && !ModAttachments.getData(animal, ModAttachments.SLEEPING) && animal.hurtTime == 0
    ...
    private boolean ownYoung(Animal young) {
        boolean mother = ModAttachments.getData(young, ModAttachments.PARENT).equals(parent.getUUID().toString());
        boolean father = AnimalInformation.formsPairBond(parent)
                && ModAttachments.getData(young, ModAttachments.FATHER).equals(parent.getUUID().toString());
    ...
    @Override public boolean canUse() {
        ...
        if (parent.isBaby() || !available(parent) || ModAttachments.getData(parent, ModAttachments.CARE_COOLDOWN) > 0
    ...
    @Override public void tick() {
        ...
        boolean nursing = FamilyLifecycle.milkDependent(child)
                && ModAttachments.getData(child, ModAttachments.PARENT).equals(parent.getUUID().toString());
    ...
    @Override public void stop() {
        ...
        ModAttachments.setData(parent, ModAttachments.CARE_COOLDOWN, FamilyLifecycle.careInterval(parent));
    }
