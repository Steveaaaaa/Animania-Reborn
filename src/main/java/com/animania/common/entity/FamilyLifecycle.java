...
public final class FamilyLifecycle {
    ...
    public static boolean milkDependent(Animal a) {
        return mammal(a) && a.isBaby() && ModAttachments.getData(a, ModAttachments.CHILD_GROWTH) < weaningStage(a);
    }
    public static Animal mother(Animal child) {
        if (!(child.level() instanceof ServerLevel level)) return null;
        try {
            var entity = level.getEntity(java.util.UUID.fromString(ModAttachments.getData(child, ModAttachments.PARENT)));
    ...
    public static boolean staysNearBirthplace(Animal child) {
        return child.isBaby() && nestYoung(child) && ModAttachments.getData(child, ModAttachments.CHILD_GROWTH) < 15
                && mother(child) != null;
    }
    public static void tick(Animal animal) {
        int poseTime = ModAttachments.getData(animal, ModAttachments.FAMILY_POSE_TTL);
        if (poseTime > 0) {
            ModAttachments.setData(animal, ModAttachments.FAMILY_POSE_TTL, poseTime - 1);
            if (poseTime == 1) FamilyAnimationState.set(animal, 0);
        }
        if (mammal(animal) && animal.tickCount % 200 == 0 && !ModAttachments.getData(animal, ModAttachments.NURSING_ID).isEmpty()
                && (!animal.isBaby() || ModAttachments.getData(animal, ModAttachments.CHILD_GROWTH) >= weaningStage(animal))) weaned(animal);
        if (animal.tickCount % 200 == 0) FamilyUpdates.receive(animal);
        int recovery = ModAttachments.getData(animal, ModAttachments.RECOVERY);
        if (recovery > 0) ModAttachments.setData(animal, ModAttachments.RECOVERY, Math.max(0, recovery - HusbandryMood.work(animal)));
        int cooldown = ModAttachments.getData(animal, ModAttachments.CARE_COOLDOWN);
        if (cooldown > 0) ModAttachments.setData(animal, ModAttachments.CARE_COOLDOWN, cooldown - 1);
        int fertile = ModAttachments.getData(animal, ModAttachments.FERTILIZED_TIMER);
        if (fertile > 0) ModAttachments.setData(animal, ModAttachments.FERTILIZED_TIMER, fertile - 1);
    }
