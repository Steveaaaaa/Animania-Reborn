...
public final class HusbandryMood {
    ...
    public static boolean managed(Animal animal) { return enabled() && ModAttachments.getData(animal, ModAttachments.CARE_LEASE) > 0; }
    public static void caredFor(Animal animal) {
        if (animal.level().isClientSide() || kind(animal) == null) return;
        if (ModAttachments.getData(animal, ModAttachments.CARE_LEASE) <= 0) {
            ModAttachments.setData(animal, ModAttachments.MOOD_SCORE, 50);
            ModAttachments.setData(animal, ModAttachments.MOOD_GRACE, AnimaniaConfig.MOOD_GRACE_TICKS.get());
            ModAttachments.setData(animal, ModAttachments.MOOD_CALENDAR, animal.level().getDayTime());
        }
        ModAttachments.setData(animal, ModAttachments.MOOD_ENROLLED, 1);
        ModAttachments.setData(animal, ModAttachments.CARE_LEASE, 48000);
    }
    public static int effect(Animal animal) {
        if (!managed(animal) || ModAttachments.getData(animal, ModAttachments.MOOD_GRACE) > 0) return 0;
        int score = ModAttachments.getData(animal, ModAttachments.MOOD_SCORE);
        return score >= 75 ? 1 : score < 35 && AnimaniaConfig.MOOD_PENALTIES.get() ? -1 : 0;
    }
    public static boolean breedingAllowed(Animal animal) {
        return effect(animal) >= 0 || ModAttachments.getData(animal, ModAttachments.MOOD_SCORE) >= 20;
    }
    ...
    public static boolean milkReady(Animal animal) { return effect(animal) >= 0 || ModAttachments.getData(animal, ModAttachments.MILK_REST) == 0; }
    public static void milked(Animal animal) { if (managed(animal)) ModAttachments.setData(animal, ModAttachments.MILK_REST, milkCooldown(animal)); }
    ...
    public static void tick(Animal animal) {
        ...
        int lease = ModAttachments.getData(animal, ModAttachments.CARE_LEASE);
        if (lease > 0) ModAttachments.setData(animal, ModAttachments.CARE_LEASE, lease - 1);
        int grace = ModAttachments.getData(animal, ModAttachments.MOOD_GRACE);
        if (grace > 0) {
            long now = animal.level().getDayTime();
            long previous = ModAttachments.getData(animal, ModAttachments.MOOD_CALENDAR);
            ...
            ModAttachments.setData(animal, ModAttachments.MOOD_GRACE, (int) Math.max(0L, grace - elapsed));
            ModAttachments.setData(animal, ModAttachments.MOOD_CALENDAR, now);
        }
        int milk = ModAttachments.getData(animal, ModAttachments.MILK_REST);
        if (milk > 0) ModAttachments.setData(animal, ModAttachments.MILK_REST, milk - 1);
        ...
        if (owned || ModAttachments.getData(animal, ModAttachments.MOOD_ENROLLED) == 0 && ModAttachments.getData(animal, ModAttachments.HAND_FED)) caredFor(animal);
        ...
        if (effect(animal) > 0 && ModAttachments.getData(animal, ModAttachments.MOOD_MISSING) == 0 && animal.tickCount % 1200 < 200)
            animal.heal(1.0F);
    }
    private static void evaluate(Animal animal, HusbandryHabitat h) {
        ...
        for (Animal other : nearby) if (kind(other) == kind) {
            companions++;
            if (animal.distanceToSqr(other) < 9 && !other.isBaby() && !other.getUUID().toString().equals(ModAttachments.getData(animal, ModAttachments.PARENT))) adults++;
        }
        ...
        ModAttachments.setData(animal, ModAttachments.MOOD_MISSING, missing);
        ...
        int score = ModAttachments.getData(animal, ModAttachments.MOOD_SCORE);
        ModAttachments.setData(animal, ModAttachments.MOOD_SCORE, score + Math.max(-2, Math.min(3, target-score)));
    }
    ...
    public static void copy(Animal from, Animal to) {
        ModAttachments.setData(to, ModAttachments.CARE_LEASE, ModAttachments.getData(from, ModAttachments.CARE_LEASE));
        ModAttachments.setData(to, ModAttachments.MOOD_ENROLLED, ModAttachments.getData(from, ModAttachments.MOOD_ENROLLED));
        ModAttachments.setData(to, ModAttachments.MOOD_SCORE, ModAttachments.getData(from, ModAttachments.MOOD_SCORE));
        ModAttachments.setData(to, ModAttachments.MOOD_GRACE, ModAttachments.getData(from, ModAttachments.MOOD_GRACE));
        ModAttachments.setData(to, ModAttachments.MOOD_CALENDAR, ModAttachments.getData(from, ModAttachments.MOOD_CALENDAR));
        ModAttachments.setData(to, ModAttachments.MOOD_MISSING, ModAttachments.getData(from, ModAttachments.MOOD_MISSING));
        ModAttachments.setData(to, ModAttachments.MILK_REST, ModAttachments.getData(from, ModAttachments.MILK_REST));
    }
}
